...
const market_url_template = 'https://www.exbitron.com/trading/scausdt';

function get_summary(coin, exchange, cb) {
  var url=base_url + '/public/markets/' + coin.toLowerCase() + exchange.toLowerCase() + '/tickers';
  request({uri: url, json: true, headers: {'User-Agent': 'scalaris'}}, function (error, response, body) {
    if (error)
...
function get_trades(coin, exchange, cb) {
  ...
  request({ uri: req_url, json: true, headers: {'User-Agent': 'scalaris'}}, function (error, response, body) {
    if (error)
      return cb(error, null);
    else if (body.error !== true) {
      var trades = [];
...
function get_chartdata(coin, exchange, cb) {
  ...
  request({ uri: req_url, json: true, headers: {'User-Agent': 'scalaris'} }, function (error, response, chartdata) {
    if (error)
      return cb(error, []);
    else {
      var processed = [];
